...
class Station
  ...
  def ingoing_train(train)
    self.train_on_station << train  #записать в списко поездов на станции   
    @@train_count += 1
  end

  def outgoing_train(train)
    self.train_on_station.delete(train)
    @@train_count -= 1
  end
  
  def train_by_type(type)
    self.train_on_station.filter_map do |train|
      if train.type == type
      else
      end
    end
...
class Train < Route
  ...
  def move_next_station         # Используй здесь метод next_station
    self.station += next_station
  end
  
  def move_previous_station     # А где метод движения назад?
    self.station -= next_station
  end
